...
先承认它有真东西。**循环是图的特例**——一根 `while` 就是"一个节点、一条自环边"的图。当任务需要多角色（规划者/执行者/审查者）、条件分支（"测试挂了就回到修复节点"）、并行扇出再汇总（4 个 subagent 同时跑再合并）时，一根线性循环表达不了，你自然会把它画成有向图。code-hacker 那个"1 主 agent + 4 subagent + 条件路由"的结构，画出来本来就是一张图。所以"用图组织 Agent"是成立的，它是循环在拓扑上的自然推广。
...
| 概念 | 它给你什么 | 真实分量 |
|---|---|---|
| **Loop** | 反馈迭代的骨架 | 最小、免费，谁都会写 |
| **Harness** | 可靠性：上下文/工具/护栏/记忆 | **昂贵、稀缺，价值重心** |
| **Graph** | 表达力：多角色/分支/并行 | 有用但不新，LangGraph 早有 |

> Loop / Harness / Graph 这三个词会一直被轮流炒。但从 autoresearch 的 `while True:` 到 code-hacker 的 62 个工具，**价值的重心始终压在中间那层 Harness 上，从没动过。**
